...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await anthropicAdapter.grade(params);
    return NextResponse.json(result);
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Anthropic grading failed';
    console.error('[/api/ai/anthropic]', err);
